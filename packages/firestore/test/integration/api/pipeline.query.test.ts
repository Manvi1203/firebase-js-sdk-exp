...
apiPipelineDescribe('Queries', (persistence, pipelineMode) => {
  ...
  it('can listen/unlisten/relisten to mirror queries', () => {
    ...
    return withTestCollection(persistence, testDocs, async collection => {
      // Setup `limit` query
      const storeLimitEvent = new PipelineEventsAccumulator<QuerySnapshot>();
      ...
      const storeLimitToLastEvent =
        new PipelineEventsAccumulator<QuerySnapshot>();
  ...
  it('maintains correct DocumentChange indexes', async () => {
    ...
    await withTestCollection(persistence, testDocs, async coll => {
      const accumulator = new PipelineEventsAccumulator<
  ...
  it.skip('can listen for the same query with different options', () => {
    const testDocs = { a: { v: 'a' }, b: { v: 'b' } };
    return withTestCollection(persistence, testDocs, coll => {
      const storeEvent = new PipelineEventsAccumulator<
        QuerySnapshot | RealtimePipelineSnapshot
      >();
      const storeEventFull = new PipelineEventsAccumulator<
        QuerySnapshot | RealtimePipelineSnapshot
      >();
  ...
  it('can listen for QueryMetadata changes', () => {
    ...
    return withTestCollection(persistence, testDocs, coll => {
      ...
      const accum = new PipelineEventsAccumulator<QuerySnapshot>();
      let unlisten2: () => void;
  ...
  it('trigger with isFromCache=true when offline', () => {
    return withTestCollection(persistence, { a: { foo: 1 } }, (coll, db) => {
      const accum = new PipelineEventsAccumulator<
        QuerySnapshot | RealtimePipelineSnapshot
      >();
      const unregister = onSnapshot(
        pipelineMode,
